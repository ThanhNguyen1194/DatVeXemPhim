    ...
    return this.dataPhone.map((item, index) => {
      return <div className='col-4' key={index}>
        <div className="card text-left">
          <img style={{width: '350px', height: '350px'}} className="card-img-top img-fluid" src={item.hinhAnh} alt={item.hinhAnh} />
          <div className="card-body">
            <h4 className="card-title">{item.tenSP}</h4>
            <p className="card-text">{item.giaBan.toLocaleString()}</p>
            <button onClick={() => {this.xemChiTiet(item)}} className="btn btn-primary">Xem chi tiết</button>
          </div>
        </div>
